...
public class SkeletonOnHorseBottles extends AbstractQuest {

    private static int BOTTLES_AMOUNT = 3;

    public SkeletonOnHorseBottles(int id, int weight, int reward) {
        this.id = id;
        this.weight = weight;
        this.reward = reward;
    }
    ...
    public TextComponent getDescription() {
        return Component.text()
                .append(Component.text("НЕ ХВАТАЕТ КОСТЕЙ!!! Мне нужны сросшиеся кости двух существ. Мне нужно, чтобы ты принес мне эссенцию скелетов на лошадях в количестве ", NamedTextColor.DARK_AQUA))
        ...
        try {
            return itemStack.getType().equals(Material.GLASS_BOTTLE) &&
                    ...
                    ((TextComponent) itemStack.getItemMeta().displayName()).content()
                            .contains(AllMobTypes.getCustomMobById(9).getMobName());
        } catch (Exception e) {
            return false;
        }
    }
