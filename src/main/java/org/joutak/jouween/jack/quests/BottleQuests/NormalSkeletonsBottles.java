...
public class NormalSkeletonsBottles extends AbstractQuest {

    private static int BOTTLES_AMOUNT = 15;

    public NormalSkeletonsBottles(int id, int weight, int reward) {
        this.id = id;
        this.weight = weight;
        this.reward = reward;
    }
    ...
    public TextComponent getDescription() {
        return Component.text()
                .append(Component.text("Не хватает костлявых. Мне нужно, чтобы ты принес мне эссенцию обычных зараженных скелетов в количестве ", NamedTextColor.DARK_AQUA))
        ...
        try {
            return itemStack.getType().equals(Material.GLASS_BOTTLE) &&
                    ...
                    ((TextComponent) itemStack.getItemMeta().displayName()).content()
                            .contains(AllMobTypes.getCustomMobById(6).getMobName());
        } catch (Exception e){
            return false;
        }
    }
